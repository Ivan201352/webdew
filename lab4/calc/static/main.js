...
function evaluate(str) {
    ...
    // иф для двух после запятой 
    if (Math.round(result) != result)
        result = result.toFixed(2);
    
    ciferki.innerHTML = '';
    ciferki.append(result);
}
...
function clickHandler(event) {
    ...
    let cifri = document.getElementById('ciferki');
    // добавляем на поле циферки значения, на которые кликнули
    for (let i = 0; i < digitsList.length; i++) {
        if (element == digitsList[i].innerHTML) {
            cifri.append(element);
        }
    }
    ...
    //Выводим результат через функцию
    if (elemClass == 'key result') {
        let str = cifri.innerHTML;
        evaluate(str);
    }
}
...
window.onload = function () {
    let butoonsDigit = document.querySelector('.digits');
    butoonsDigit.addEventListener('click', clickHandler);
    let buttonsOther = document.querySelector('.other');
    buttonsOther.addEventListener('click', clickHandler);
}